...
import (
	...
	"github.com/irfansharif/fraft"
	"github.com/irfansharif/fraft/raftpb"
...
type node struct {
	...
	// stable
	storage *fraft.MemoryStorage

	mu    sync.Mutex // guards state
	state raftpb.HardState
}

func startNode(id uint64, peers []fraft.Peer, iface iface) *node {
	st := fraft.NewMemoryStorage()
	c := &fraft.Config{
		ID:              id,
		ElectionTick:    10,
		HeartbeatTick:   1,
	...
	go func() {
		for {
			...
			case <-ticker:
				n.Tick()
			case rd := <-n.Ready():
				if !fraft.IsEmptyHardState(rd.HardState) {
					n.mu.Lock()
